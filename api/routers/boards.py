...
from api.crud.boards import create_board, delete_board, get_board, get_boards, update_board
...
async def create_board_endpoint(
    body: BoardCreate, db: AsyncSession = Depends(get_db)
) -> BoardResponse:
    """Create a new board."""
...
async def update_board_endpoint(
    board_id: str, body: BoardUpdate, db: AsyncSession = Depends(get_db)
) -> BoardResponse:
    """Update a board."""
    board = await get_board(db, board_id)
    if board is None:
    ...
    board = await update_board(
        db, board, name=body.name, description=body.description, archived=body.archived
    )
    return BoardResponse(
        id=board.id,
        name=board.name,
